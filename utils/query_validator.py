    ...
    if where_match:
        ...
        if not clause.startswith('('):
            ...
            if len(parts) > 1:
                ...
                while i < len(parts):
                    if i % 2 == 0:
                        ...
                        if field_match:
                            ...
                            j = i + 2
                            while j < len(parts) and j - 1 < len(parts):
                                operator = parts[j - 1].strip() if j - 1 < len(parts) else None
                                if j < len(parts):
                                    next_cond = parts[j].strip()
                                    next_field_match = re.match(r'(\w+)\s*[<>=!]', next_cond)
                                    if next_field_match and next_field_match.group(1) == field:
                                        group.append(operator)
                                        group.append(next_cond)
                                        j += 2
                                    else:
                            ...
                            formatted_parts.append('(' + ' '.join(group) + ')')
                            
                            if operator_to_add and j < len(parts):
                                formatted_parts.append(operator_to_add)
                                operator_to_add = None
                            
                            i = j
                        else:
                            formatted_parts.append(f'({condition})')
                            if i + 1 < len(parts):
                ...
                new_clause = ' '.join(formatted_parts)
                formatted = formatted.replace(clause, f'({new_clause})')
                warnings.append("⚠️ Added nested parentheses for grouped conditions")
